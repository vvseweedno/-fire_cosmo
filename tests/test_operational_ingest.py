...
def test_observation_rejects_non_datetime_timestamp():
    observation = _obs()
    object.__setattr__(observation, "acquired_at", None)
...
def test_observation_rejects_timezone_without_utc_offset():
    ...
    invalid = _obs(acquired_at=datetime(2026, 7, 1, 10, 0, tzinfo=MissingOffsetTimezone()))
...
@pytest.mark.parametrize(
    ("bbox", "message"),
    [
        ((0.0, 0.0, 1.0), "four-coordinate tuple"),
        ([0.0, 0.0, 1.0, 1.0], "four-coordinate tuple"),
        ((0.0, 0.0, "1", 1.0), "coordinates must be numeric"),
        ((0.0, 0.0, True, 1.0), "coordinates must be numeric"),
    ],
)
def test_observation_rejects_malformed_bbox_metadata(bbox, message):
    observation = _obs()
    object.__setattr__(observation, "bbox", bbox)
...
def test_observation_rejects_non_finite_bbox_coordinates(non_finite):
    observation = _obs(crs="EPSG:3857", bbox=(0.0, 0.0, non_finite, 1.0))
...
def test_capabilities_separate_public_and_implemented_sensor_families():
    ...
    assert capabilities["active_fire"]["public_sensor_families"] == ["MODIS", "VIIRS", "LANDSAT"]
    assert capabilities["active_fire"]["implemented_inference_adapters"] == ["VIIRS"]
    assert capabilities["burn_assessment"]["implemented_inference_adapters"] == ["SENTINEL-2"]


def test_capability_channel_requirements_match_readiness_contract():
    capabilities = operational_capabilities()
    assert capabilities["active_fire"]["implemented_viirs_minimum_channels"] == ["I4", "I5"]
    assert capabilities["burn_assessment"]["implemented_sentinel2_minimum_channels"] == ["B12", "B8A"]
...
def test_build_burn_pair_requires_ordered_coregistered_sentinel2():
    pre = _obs(observation_id="pre", sensor="Sentinel-2", channels=("B8A", "B12"), acquired_at=datetime(2026, 6, 1, 9, 0, tzinfo=UTC))
    post = _obs(observation_id="post", sensor="S2", channels=("B8A", "B12"), acquired_at=datetime(2026, 7, 1, 9, 0, tzinfo=UTC))
...
def test_build_burn_pair_rejects_same_observation_id():
    pre = _obs(observation_id="same-scene", sensor="Sentinel-2", channels=("B8A", "B12"), acquired_at=datetime(2026, 6, 1, tzinfo=UTC))
    post = _obs(observation_id="same-scene", sensor="Sentinel-2", channels=("B8A", "B12"), acquired_at=datetime(2026, 7, 1, tzinfo=UTC))
...
def test_build_burn_pair_rejects_reversed_time():
    pre = _obs(observation_id="pre", sensor="Sentinel-2", channels=("B8A", "B12"), acquired_at=datetime(2026, 8, 1, tzinfo=UTC))
    post = _obs(observation_id="post", sensor="Sentinel-2", channels=("B8A", "B12"), acquired_at=datetime(2026, 7, 1, tzinfo=UTC))
    with pytest.raises(ValueError, match="earlier"):
        build_burn_pair(pre, post)


def test_build_burn_pair_rejects_non_coregistered_bbox():
    pre = _obs(observation_id="pre", sensor="Sentinel-2", channels=("B8A", "B12"), acquired_at=datetime(2026, 6, 1, tzinfo=UTC))
    post = _obs(observation_id="post", sensor="Sentinel-2", channels=("B8A", "B12"), acquired_at=datetime(2026, 7, 1, tzinfo=UTC), bbox=(38.1, 45.0, 39.1, 46.0))
